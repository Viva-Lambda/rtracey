...
struct Box {
  ...
  __host__ __device__ void minmax_points(Point3 &p1,
    ...
    float *p0x = (float *)p_1.hparam.p1x; // p0x
    float *p1x = (float *)p_1.hparam.p1y; // p1x
    float *p0y = (float *)p_1.hparam.p2x; // p0y
    float *p1y = (float *)p_1.hparam.p2y; // p1y

    float *p0z = (float *)p_2.hparam.radius; // p0z
    float *p1z = (float *)p_1.hparam.radius; // p1z
...
struct ConstantMedium {
  ...
  __device__
  ConstantMedium(const Primitive *&b, const float *d,
                 const TextureParam &a, const int *gsize,
                 curandState *s)
      : boundary(b), neg_inv_density(-1 / (*d)),
