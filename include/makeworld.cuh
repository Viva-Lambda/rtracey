...
SceneObjects make_cornell_box() {
  ...
  int pcount = 0;
  prms[pcount] =
      ScenePrim sp_green(green_param, green_wall, 0, 0);
  pcount++;
  prms[pcount] =
      ScenePrim sp_red(red_param, red_wall, 1, 0);
  pcount++;
  prms[pcount] =
      ScenePrim sp_light(light_param, light_wall, 2, 0);
  pcount++;
  prms[pcount] =
      ScenePrim sp_white(white_param, white_wall, 3, 0);
  pcount++;
  prms[pcount] =
      ScenePrim sp_white2(white_param, white_wall2, 4, 0);
  pcount++;
  prms[pcount] =
      ScenePrim sp_blue(blue_param, blue_wall, 5, 0);
  SceneGroup sg(prms, prim_count, 0, SOLID);
  return sg;
}

__global__ void make_empty_c_box(Hittables **world,
                                 SceneObjects sobjs) {
  //
